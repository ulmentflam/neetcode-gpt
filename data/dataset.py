...
class Solution:
    def batch_loader(self, raw_dataset: str, context_length: int, batch_size: int) -> Tuple[List[List[str]], List[List[str]]]:
        ...
        torch.manual_seed(0)
        ws_tokens = raw_dataset.split()
        ix = torch.randint(len(ws_tokens) - context_length, (batch_size,))
        X = [ws_tokens[i:i+context_length] for i in ix]
        Y = [ws_tokens[i+1:i+1+context_length] for i in ix]
        return X, Y
